 ...
 */
final class Version20210430160755 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__message_chat AS SELECT id, chat_id, content, created_at FROM message_chat');
        $this->addSql('DROP TABLE message_chat');
        $this->addSql('CREATE TABLE message_chat (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, chat_id INTEGER DEFAULT NULL, content VARCHAR(255) NOT NULL COLLATE BINARY, created_at DATETIME NOT NULL, author_msg VARCHAR(255) NOT NULL, CONSTRAINT FK_CC0869731A9A7125 FOREIGN KEY (chat_id) REFERENCES chat (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO message_chat (id, chat_id, content, created_at) SELECT id, chat_id, content, created_at FROM __temp__message_chat');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__message_chat AS SELECT id, chat_id, content, created_at FROM message_chat');
        $this->addSql('DROP TABLE message_chat');
        $this->addSql('CREATE TABLE message_chat (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, chat_id INTEGER DEFAULT NULL, content VARCHAR(255) NOT NULL, created_at DATETIME NOT NULL)');
        $this->addSql('INSERT INTO message_chat (id, chat_id, content, created_at) SELECT id, chat_id, content, created_at FROM __temp__message_chat');
        $this->addSql('DROP TABLE __temp__message_chat');
        $this->addSql('CREATE INDEX IDX_CC0869731A9A7125 ON message_chat (chat_id)');
        $this->addSql('DROP INDEX IDX_5A8A6C8DF675F31B');
